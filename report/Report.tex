...
\begin{lstlisting}[
	...
	frame = l,
	numbers=left,
	caption = {Registration of the I2C driver (from \emph{lis3lv02d-nhk8815.c}).},
...
settings, as explained in section \ref{sec:lis3_dev_settings}.
...
the device identification number (3A for the LIS3LV02D chip)}.
...
cpu while the LIS3LV02D inertial sensor is the device being polled.

To implement the polling feature the LIS3LV02D driver registers itself with the
...
Upon loading the LIS3LV02D driver, the polling function may be either enable
or not, depending on whether the default or custom settings were used
(see section \ref{sec:lis3_dev_settings} for details).
...
\label{sec:lis3_poll}
The LIS3LV02D may be configured to generate an inertial wake-up/free-fall
interrupt signal when a programmable acceleration threshold is crossed at least
in one of the three axes.

The LIS3LV02D driver installs a threaded ISR to handle the free-fall detection.
...
%-------------------------------------------------------------------------------
\subsection{LIS3LV02D driver settings}
\label{sec:lis3_dev_settings}

The customization of the LIS3LV02D settings is done based on \emph{platform data}.
...
(listing \ref{list:lis3-paltform-data}) which allows to set a number of LIS3LV02D parameters including the acceleration full-range (2g or 6g), free-fall settings,
...
\begin{lstlisting}[
	...
	frame = l,
	numbers = left,
	caption = {LIS3LV02D driver's platform data
...
\begin{lstlisting}[
	...
	frame = l,
	numbers = left,
	caption = {Loading of the driver settings
...
The driver's default settings are shown below.

\begin{lstlisting}[
	xleftmargin=15pt,
	frame = l,
	numbers = left,
	caption = {LIS3LV02D driver default setup
